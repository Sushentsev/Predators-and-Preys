...
class Buffer(object):
    def __init__(self, state_dim: int, action_dim: int, max_size: int):
        ...
        self.state_buff = np.zeros((max_size, state_dim))
        self.action_buff = np.zeros((max_size, action_dim))
        self.reward_buff = np.zeros(max_size)
        self.next_state_buff = np.zeros((max_size, state_dim))
        self.done_buff = np.zeros(max_size)
    ...
    def sample(self, batch_size: int, norm_rew: bool = False) -> Sequence[np.ndarray]:
        indices = np.random.choice(self.curr_size, batch_size, replace=False)

        if norm_rew:
            mean = np.mean(self.reward_buff[:self.curr_size])
            std = np.std(self.reward_buff[:self.curr_size])
            rew = (self.reward_buff[indices] - mean) / std
        else:
            rew = self.reward_buff[indices]

        return self.state_buff[indices], self.action_buff[indices], \
               rew, self.next_state_buff[indices], self.done_buff[indices]
